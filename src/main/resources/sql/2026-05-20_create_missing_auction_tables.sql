-- Creates the auction tables without dropping existing data.
...
create index if not exists idx_auction_work on tbl_auction (work_id);
create index if not exists idx_auction_seller on tbl_auction (seller_id);
create index if not exists idx_auction_status on tbl_auction (status, closing_at);
create index if not exists idx_auction_closing on tbl_auction (closing_at);
...
create index if not exists idx_bid_auction on tbl_bid (auction_id, bid_price desc);
create index if not exists idx_bid_member  on tbl_bid (member_id);

create table if not exists tbl_auction_wishlist (
    id         bigint generated always as identity primary key,
    auction_id bigint    not null,
    member_id  bigint    not null,
    created_datetime timestamp not null default now(),
...
create index if not exists idx_aw_member on tbl_auction_wishlist (member_id);

select
    to_regclass('public.tbl_auction') as tbl_auction,
    to_regclass('public.tbl_bid') as tbl_bid,
    to_regclass('public.tbl_auction_wishlist') as tbl_auction_wishlist;
