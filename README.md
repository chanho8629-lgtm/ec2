...
| **핵심 기여** | 작품 CRUD, 갤러리(예술관) CRUD, 경매, Bootpay 결제, AWS S3 파일 저장 |
| **담당 범위** | API·서비스·MyBatis 데이터 처리, 화면 연동, 외부 결제 검증, S3 업로드 및 URL 변환 |
...
> 팀 프로젝트의 전체 기능은 아래에 정리되어 있으며, 위 항목은 정찬호가 직접 구현한 핵심 영역입니다.
